...
class MainWindow(QMainWindow):
    ...
    def _build_settings_tab(self) -> QWidget:
        ...
        self.settings_fields = {
            ...
            "spreadsheet_enabled": QLineEdit(),
            "workbook_path": QLineEdit(),
            "worksheet_name": QLineEdit(),
            "field_routes_json": QLineEdit(),
        }
        ...
        self._add_form_row_help(form, "Spreadsheet export enabled", self.settings_fields["spreadsheet_enabled"], "true/false. If true, mapped Woo fields are exported to workbook.")
        self._add_form_row_help(form, "Workbook path", self.settings_fields["workbook_path"], "Path to .xlsx workbook used for Woo field routing export.")
        self._add_form_row_help(form, "Worksheet name", self.settings_fields["worksheet_name"], "Target worksheet/tab in workbook.")
        self._add_form_row_help(form, "Spreadsheet field routes JSON", self.settings_fields["field_routes_json"], "JSON list: [{woo_field,target_column,enabled}]")
    ...
    def _load_settings_into_fields(self) -> None:
        ...
        self.settings_fields["spreadsheet_enabled"].setText(str(self.engine.settings.spreadsheet.enabled).lower())
        self.settings_fields["workbook_path"].setText(self.engine.settings.spreadsheet.workbook_path)
        self.settings_fields["worksheet_name"].setText(self.engine.settings.spreadsheet.worksheet_name)
        self.settings_fields["field_routes_json"].setText(
            json.dumps([r.model_dump() for r in self.engine.settings.spreadsheet.field_routes])
        )

        available = ", ".join(SpreadsheetRouter.available_woo_fields())
        self.settings_error_label.setText(f"Available Woo fields for routing: {available}")
        self.settings_error_label.setStyleSheet("color: #334155;")
    ...
    def _validate_settings_form(self) -> bool:
        ...
        spreadsheet_enabled_raw = require("spreadsheet_enabled", "Spreadsheet export enabled")
        require("workbook_path", "Workbook path")
        require("worksheet_name", "Worksheet name")
        field_routes_json = require("field_routes_json", "Spreadsheet field routes JSON")
        ...
        if spreadsheet_enabled_raw.lower() not in {"true", "false"}:
            self.field_errors["spreadsheet_enabled"] = "Spreadsheet export enabled must be true or false."

        try:
            route_rows = json.loads(field_routes_json)
            if not isinstance(route_rows, list):
                raise ValueError("Field routes must be a JSON list")
            for row in route_rows:
                if not isinstance(row, dict):
                    raise ValueError("Each field route must be an object")
                if "woo_field" not in row:
                    raise ValueError("Each field route needs woo_field")
                if row.get("woo_field") not in SpreadsheetRouter.available_woo_fields():
                    raise ValueError(f"Unsupported woo_field: {row.get('woo_field')}")
                if "target_column" not in row:
                    raise ValueError("Each field route needs target_column")
        except Exception as exc:  # noqa: BLE001
            self.field_errors["field_routes_json"] = f"Spreadsheet field routes JSON invalid: {exc}"
    ...
    def _save_and_apply_settings(self) -> None:
        ...
        try:
            ...
            self.engine.settings.spreadsheet.enabled = self.settings_fields["spreadsheet_enabled"].text().strip().lower() == "true"
            self.engine.settings.spreadsheet.workbook_path = self.settings_fields["workbook_path"].text().strip()
            self.engine.settings.spreadsheet.worksheet_name = self.settings_fields["worksheet_name"].text().strip()
            self.engine.settings.spreadsheet.field_routes = [
                SpreadsheetFieldRoute.model_validate(row)
                for row in json.loads(self.settings_fields["field_routes_json"].text().strip())
            ]
            if self.engine.spreadsheet_router is not None:
                self.engine.spreadsheet_router.settings = self.engine.settings.spreadsheet
